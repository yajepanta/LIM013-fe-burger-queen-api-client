...
const Card = ({ status, calculateDate, props, modifyOrder }) => {
  return (
    <div className="card">
      <div className="row">
        {status === "delivering" ? `Tiempo total: ${calculateDate}` : ""}
      </div>
      <div className="card-title flex">
        <span>N°001</span>
        <span
          className={status === "pending" ? "chip-estado" : "chip-estado green"}
        >
          {status === "pending" ? "Pendiente" : "Entregando"}
        </span>
      </div>
      <hr></hr>
      ...
      <button className="btn-send" onClick={(e) => modifyOrder(props._id)}>
        Enviar
      </button>
    </div>
  );
